...
const Index = () => {
  ...
  // Use auth user data if profile is not available
  const actualUserRole = profile?.role || 'member';
  const userName = profile?.name || authUser?.user_metadata?.name || authUser?.email || 'Guest';
