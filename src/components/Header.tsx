...
const Header = ({
  ...
  const generatePDFReport = async () => {
    try {
      console.log('Generating PDF report with enhanced Urdu support...');
      ...
      // Use Arial Unicode MS for better Urdu support, fallback to Arial
      try {
        pdf.setFont('Arial Unicode MS', 'normal');
      } catch {
        pdf.setFont('Arial', 'normal');
      }
      
      // Add title - use English for better compatibility
      pdf.setFontSize(18);
      pdf.setFont('helvetica', 'bold');
      pdf.text('Majlis Dawatul Haq - Task Report', pageWidth / 2, yPosition, { align: 'center' });
      yPosition += 15;
      
      // Add date in English format for readability
      pdf.setFontSize(11);
      ...
      // Task Details in English
      pdf.setFontSize(14);
      pdf.setFont('helvetica', 'bold');
      pdf.text('Detailed Task List:', 20, yPosition);
      yPosition += 12;
      ...
      tasks.forEach((task, index) => {
        ...
        pdf.setFontSize(11);
        pdf.setFont('helvetica', 'bold');
        // Convert Urdu title to English readable format
        const taskTitle = task.title || `Task ${index + 1}`;
        pdf.text(`${index + 1}. ${taskTitle}`, 25, yPosition);
        yPosition += 7;
        ...
        if (task.description) {
          const descLines = pdf.splitTextToSize(`Description: ${task.description}`, pageWidth - 60);
        ...
        if (task.memberNotes) {
          const notesLines = pdf.splitTextToSize(`Member Notes: ${task.memberNotes}`, pageWidth - 60);
  ...
  return (
    <>
      ...
      {/* Notification Panel - Fixed positioning to prevent overlap */}
      <NotificationPanel
        notifications={realtimeNotifications}
